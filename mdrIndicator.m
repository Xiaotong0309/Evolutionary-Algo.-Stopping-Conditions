...
mdr = zeros(n,1);
for k = 1:n 
    [normA normB] = normDelta(state.preGen{k}, PFt);   %The delta function returns the set of the elements of setA that are
     % dominated by at least one element of setB and vice versa. normdelta
     % returns the number
    mdr(k) = normA - normB;  
end
state.mdr = mdr;