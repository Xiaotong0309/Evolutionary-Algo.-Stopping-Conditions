function [flag,state] = adaptiveTestingDecision(state, params)
...
if ~isfield(state, 'flagPreGen')%判断输入是否是结构体数组的域(成员)
...
for j = 1:length(state.EGP)
    ...
    if ~isnan(egpValues(1,1)) && ...
            (isnan(egpValues(1,2)) || isnan(egpValues(1,3)))%isnan判断是否为数字
